...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        layout.addWidget(self.qr_text_label, 3, 1, 1, 2)
        ...
        self.pay_button.clicked.connect(self.show_qrcode_screen)
    ...
    def show_qrcode_screen(self):
        # gnerste the invoice and set the QR code image
        if self.invoice is None:
            self.invoice = utilities.generate_invoice(self.total_eur)
        utilities.generate_qr_code(self.invoice, self.qr_label)

        self.qr_text_label.setText("Scanne den QR Code.")
